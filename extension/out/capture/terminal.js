...
exports.registerTerminalCapture = registerTerminalCapture;
const vscode = __importStar(require("vscode"));
// Known test runner patterns
const TEST_RUNNER_RE = /\b(pytest|npm\s+test|yarn\s+test|go\s+test|jest|mvn\s+test|cargo\s+test|npx\s+jest|vitest|mocha)\b/i;
const PASS_OUTPUT_RE = /\b(\d+)\s+passed\b/i;
const FAIL_OUTPUT_RE = /\b(\d+)\s+(failed|error)/i;
const MAX_OUTPUT_CHARS = 4000;
function registerTerminalCapture(ctx, buffer, isEnabled) {
    // Capture output per-execution
    ctx.subscriptions.push(vscode.window.onDidStartTerminalShellExecution(async (e) => {
        if (!isEnabled()) {
            return;
        }
